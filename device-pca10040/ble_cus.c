...
static void on_write(ble_cus_t *p_cus, ble_evt_t const *p_ble_evt)
{
                                          ...
                                          const ble_cus_init_t *p_cus_init)
    {
        ...
        cccd_md.write_perm =
            p_cus_init->custom_value_char_attr_md.cccd_write_perm;
     ...
     * @return      NRF_SUCCESS on success, otherwise an error code.
     */
    static uint32_t tx_char_add(ble_cus_t * p_cus,
                                const ble_cus_init_t *p_cus_init)
    {
        uint32_t            err_code;
        ble_gatts_char_md_t char_md;
     ...
     * @return      NRF_SUCCESS on success, otherwise an error code.
     */
    static uint32_t rx_char_add(ble_cus_t * p_cus,
                                const ble_cus_init_t *p_cus_init)
    {
        uint32_t            err_code;
        ble_gatts_char_md_t char_md;
                                     ...
                                     uint8_t data_len)
    {
        ...
        else
        {
            ...
            NRF_LOG_INFO(
                "sd_ble_gatts_hvx result: NRF_ERROR_INVALID_STATE. \r\n");
        }
    ...
    {
        NRF_LOG_INFO("In ble_cus_steerer_value_update. \r\n");
        if (p_cus == NULL)
        {
            return NRF_ERROR_NULL;
        }
        ...
        err_code = sd_ble_gatts_value_set(p_cus->conn_handle,
                                          p_cus->steerer_handles.value_handle,
                                          &gatts_value);
        if (err_code != NRF_SUCCESS)
        {
            return err_code;
